"""
plot_new_data_compare_result.py
Author: Yan An
...
def generate_learning_curves():
    # Updated to match the folders in your 'extension_compare' directory
    datasets = ['bloodmnist', 'pathmnist']
    models = ['vit_pretrained', 'cnn']
    
    # Updated base directory based on your screenshot
    base_dir = os.path.join("outputs", "extension_compare")
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            ax_loss.plot(epochs, df['val_loss'], label=f'{model} (Validation)', 
                         color=color_map[model], linestyle='--', linewidth=2)
            
            # Plot Training and Validation Accuracy
            ax_acc.plot(epochs, df['train_acc'], label=f'{model} (Train)', 
                        color=color_map[model], linestyle='-', linewidth=2)
            ax_acc.plot(epochs, df['val_acc'], label=f'{model} (Validation)', 
                        color=color_map[model], linestyle='--', linewidth=2)
